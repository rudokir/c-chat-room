...
void handle_create(Client *sender, Client *clients, ChatRoom *rooms, char *params) {
    ...
    for (int i = 0; i < MAX_ROOMS; i++) {
        if (!rooms[i].active) {
            strncpy(rooms[i].name, params, ROOM_NAME_SIZE - 1);
            ...
            strncpy(join_params, params, ROOM_NAME_SIZE - 1);
...
void handle_msg(Client *sender, Client *clients, ChatRoom *rooms __attribute__((unused)), char *params) {
    ...
    for (int i = 0; i < MAX_CLIENTS; i++) {
        if (clients[i].fd != -1 && strcasecmp(clients[i].name, target_name) == 0) {
            ...
            strncpy(pm_content, message, max_content_size);
...
void handle_nick(Client *sender, Client *clients, ChatRoom *rooms __attribute__((unused)), char *params) {
    ...
    strncpy(old_name, sender->name, NAME_SIZE - 1);

    strncpy(sender->name, params, NAME_SIZE - 1);
...
void init_chat_rooms(ChatRoom *rooms) {
    ...
    strncpy(rooms[0].name, DEFAULT_ROOM, ROOM_NAME_SIZE - 1);
    rooms[0].active = 1;
            ...
            for (int i = 0; i < MAX_CLIENTS; i++) {
                if (clients[i].fd == -1) {
                    ...
                    if (bytes_received > 0) {
                        ...
                        // Use init_client instead of manual initialization
